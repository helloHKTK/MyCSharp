...
namespace ConsoleApplication2
{
    class Program
    {
        ...
        public static byte[] getsendCollector(byte ID01, byte ID02, byte PFC01, byte PFC02, byte value08, byte value09, byte value10, byte value11)
        {
            ...
            //将得到的十进制转换为十六进制
            char[] bytData2 = new char[16];
            Console.WriteLine("十六进制结果：");
            for (int i = 0; i < 16; i++)
            {
                if (i >= 12 && i <= 14)//CRC这三位已经转过十六进制了，不需要再转了
                {
                ...
                string strA = bytData[i].ToString("x2");
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine();
            Console.WriteLine("十进制结果(CRC那三位不准,那三位是十六进制)：");
            for (int i = 0; i < 16; i++)
            {
                Console.Write(bytData[i]+"  ");
            }
